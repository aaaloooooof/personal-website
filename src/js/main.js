		...
		setTimeout(() => {
			...
			if (canvas) {
				const gridAnimation = new GridAnimation(canvas, {
					...
					specialBlockColor: "rgba(100, 255, 152, 0)",
					specialHoverColor: "rgba(29, 202, 29, 0)",
					// 蛇身颜色渐变配置
...
function setupPortfolioInteractions() {
	if (!isPhone) {
		...
		document.body.classList.add("mouse-ready");
		window.addEventListener("mousemove", (event) => {
			glow.style.transform = `translate3d(${event.clientX}px, ${event.clientY}px, 0)`;
		});
		window.addEventListener("mouseleave", () => {
			document.body.classList.remove("mouse-ready");
		});
...
window.addEventListener("DOMContentLoaded", setupPortfolioInteractions);
